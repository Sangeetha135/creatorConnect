...
const submitContent = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const isAuthorized = campaign.applications.some(
            app => app.influencer.toString() === req.user._id.toString() && app.status === 'accepted'
        );
...
const getCampaignContent = asyncHandler(async (req, res) => {
    ...
    if (req.user.role === 'influencer' && !campaign.applications.some(
        app => app.influencer.toString() === req.user._id.toString() && app.status === 'accepted'
    )) {
        res.status(403);
        throw new Error('Not authorized to view this campaign\'s content');
...
const createContent = async (req, res) => {
    try {
        const { campaignId, deliverableType, platform, contentUrl, thumbnailUrl, caption, hashtags, mentions } = req.body;
        ...
        if (!campaign.influencers.includes(req.user._id)) {
            return res.status(403).json({ message: 'You are not authorized to submit content for this campaign' });
        }
...
const updateContentStatus = async (req, res) => {
    try {
        ...
        const isAuthorized = campaign.brand.equals(req.user._id) || content.creator.equals(req.user._id);
        if (!isAuthorized) {
...
const updateMetrics = async (req, res) => {
    try {
        ...
        if (!content.creator.equals(req.user._id)) {
            return res.status(403).json({ message: 'Not authorized to update metrics' });
        }
